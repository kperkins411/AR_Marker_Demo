...
bool MarkerDetector::findMarkers(const cv::Mat& bgraMat, std::vector<Marker>& detectedMarkers)
{
 /*   cv::Mat bgraMat(frame.height, frame.width, CV_8UC4, frame.data, frame.stride);*/
    ...
    // Detect contours, final value m_grayscaleImage.cols / 5 seems to be made up 
...
//minContourPointsAllowed seems arbitray for this call is number columns /5
void MarkerDetector::findContours(cv::Mat& thresholdImg, ContoursVector& contours, int minContourPointsAllowed) const
{
    ...
    for (size_t i=0; i<allContours.size(); i++)
    {
		...
		//where arbitrary value is applied, too small too bad
...
) 
{
    ...
    for (size_t i=0; i<contours.size(); i++)
    {
		...
		//eps gives a minimum ditance per arc, the smaller it is the more likely you will include noise see OpenCV evernote article on this
		
		//shouldn't the following line be 
		float perimeter = arcLength(contours[i],true);	//total perimeter
		float eachsidelength = perimeter/4;				//4 equal sides
		double eps = eachsidelength*0.05;
        // original line, why should eps = 0.05 times number of points
        ...
        // And they have to be convex (no bending inward)
        if (!cv::isContourConvex(approxCurve))
            continue;
//I think the following test is pointless 
//you find the min SQUARED length of a side and then see if that is greater than 
//the standard marker length (100) of course it will be its the SQUARE of the length not the length
		...
		//this check determines minimum size of marker you can detect
		//if you want to detect seen markers with sides smaller than m_minContourLengthAllowed
		//like those viewed at a distance then you cannot use correct calc here
		//need this below
		//float squaredm_minContourLengthAllowed = m_minContourLengthAllowed*m_minContourLengthAllowed;
  //      if (minDist < squaredm_minContourLengthAllowed)
  //          continue;
...
///////////////
		...
		//points need to be in same order (clock counter clock for comparison in solvePnP
...
//#define SHOW_DEBUG_IMAGES_recognizeMarkers
#ifdef SHOW_DEBUG_IMAGES_recognizeMarkers
        {
...
#ifdef SHOW_DEBUG_IMAGES_IN_WINDOW
    {
...
void MarkerDetector::estimatePosition(std::vector<Marker>& detectedMarkers)
{
    ...
    {					
		...
		//std::cout << "M = " << m.transformation.r().mat << std::endl;
